...
FullTextSearch.prototype.renderFullTextSearchResult = function(data) {
    ...
    if(data['feed_url']){
        // Change RSS URL
        $("#questions-rss-button").attr("href", data['feed_url']);
    }
...
FullTextSearch.prototype.setActiveSortTab = function(sort_method, query_string) {
    ...
    tabs.removeClass('btn-info').addClass('btn-default');
    var baseUrl = this._baseUrl;
    ...
    var arrow = (sense == 'asc' ? 'up' : 'down');
    var active_tab = $('#by_' + name);
    active_tab.removeClass('btn-default').addClass('btn-info');
    active_tab.attr(
        'title',
        askbot['data']['sortButtonData'][name][antisense + '_tooltip']
    );
    active_tab.html(
        askbot['data']['sortButtonData'][name]['label']
    );
    active_tab.append('&nbsp;<span class="glyphicon glyphicon-chevron-'+arrow+'"></span>');
};
